...
namespace Ecc.Logic.Services
{
    public class EccGetMasksService
    {
        ...
        public static string ProcessTextViaFunctions(string text, string interactionId, ICrocoAmbientContext ambientContext, Dictionary<string, IEccTextFunctionInvoker> funcs)
        {
            ...
            foreach (var replacing in replacings)
            {
                if (replacing.Func != null && funcs.ContainsKey(replacing.Func.Name))
                {
                    ...
                    text = ReplaceFirst(text, replacing.TextToReplace, func.ProccessText(interactionId, replacing, ambientContext));
                }
            }
